...
Status: verified against the working tree, 2026-08-09. Every claim below cites a
...
| Package                   | Role                                                                                                                                                                                                                                                                                                              | State                                                                                                 |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `packages/editor`         | **The editor package.** `kernel/` subpath: authored document, commands, transactions, history, tools, clipboard, grid, point types, booleans, compounds — zero React, zero DOM. `ui/` subpath: chrome (primitives kit, editor primitives, harness, canvas stage, keyboard bindings, overlay/persistence/autosave) | **Current**, 335 tests                                                                                |
| `packages/scene-model`    | Legacy `Scene` v1 wire format + spatial index                                                                                                                                                                                                                                                                     | **Transitional** — compatibility format, 7 tests                                                      |
| `packages/scene-renderer` | Draw protocol v5 (v1–v4 accepted), WASM bridge, failure policy, glass surface budget, chrome glass budget, path/overlay command channels (`.` subpath); Rust encoder + module-owned Vello wgpu renderer (device, surface, present) + glass pyramid/composite (`./wasm` subpath, crate in `rust/`)                 | **Current**, 98 vitest + 54 cargo tests                                                               |
...
The Rust crate builds via `scripts/build-scene-renderer-wasm.mjs`
(`cargo build --target wasm32-unknown-unknown --release` then `wasm-bindgen
--target web`), and requires `cargo` and `wasm-bindgen` on the machine.
