...
const overlay = (base, changes) => ({
  ...
  exists: async (relative) =>
    relative in changes ? changes[relative] !== null : base.exists(relative),
  read: async (relative) =>
    relative in changes ? changes[relative] : base.read(relative),
...
test("verification inventory fails closed for a new package and an unclassified script or export", async (context) => {
  ...
  await context.test("new package", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "packages/unreviewed/package.json":
              '{"name":"unreviewed","private":true}\n',
          }),
        ),
      "VERIFICATION_PACKAGE_INVENTORY",
    ),
  );
  await context.test("new nested package", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/plugin/opencode2/tools/unreviewed/package.json":
              '{"name":"unreviewed-nested","private":true}\n',
          }),
        ),
      "VERIFICATION_PACKAGE_INVENTORY",
    ),
  ...
  await context.test("script", async () => {
    const manifest = JSON.parse(
      await repository.read("packages/ui/package.json"),
    );
...
test("verification inventory fails closed for a new test, a skipped expected profile, and Cargo drift", async (context) => {
  const inventory = await load();
  const repository = createFileRepository(ROOT);
  await context.test("test", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/runtime/tests/unreviewed.test.ts":
  ...
  await context.test("profile mapping", async () => {
    ...
    const expected = changed.tests.find(({ profiles }) =>
      profiles.includes("network-denied"),
    );
    expected.profiles = expected.profiles.filter(
      (profile) => profile !== "network-denied",
    );
    await rejects(
      () => verifyInventory(changed, repository),
      "VERIFICATION_TEST_PROFILE",
    );
  });
    ...
    await rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/runtime/native/Cargo.toml": cargo.replace(
              "admin = []",
              "unreviewed = []",
            ),
          }),
  ...
  await context.test("implicit Cargo build script", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/runtime/native/build.rs": "fn main() {}\n",
          }),
        ),
      "VERIFICATION_CARGO_SURFACE",
    ),
...
test("ABI and workflow action, condition, and aggregate drift fail closed", async (context) => {
  assert.throws(
    () =>
      verifyAbiSymbols({
        profile: "default",
        output: "T curiosity_runtime_v0_web_search\n",
      }),
  ...
  for (const [name, changed] of [
    ...
    [
      "aggregate",
      workflow.replace(
        'test "$RUNTIME_PORTABLE" = success',
        'test "$RUNTIME_PORTABLE" != failure',
      ),
    ],
  ])
    await context.test(name, () =>
      rejects(
        () =>
          verifyInventory(
            inventory,
            overlay(repository, { ".github/workflows/opencode2.yml": changed }),
          ),
  ...
  await context.test(
    "write-all remains forbidden after an inventory hash refresh",
    async () => {
      const changedSource = workflow.replace(
        "permissions:\n  contents: read",
        "permissions: write-all",
      );
      const changedInventory = structuredClone(inventory);
      changedInventory.workflows[0].sha256 = createHash("sha256")
      ...
      await rejects(
        () =>
          verifyInventory(
            changedInventory,
            overlay(repository, {
              ".github/workflows/opencode2.yml": changedSource,
            }),
          ),
  ...
  await context.test(
    "job write permission remains forbidden after an inventory hash refresh",
    async () => {
      const changedSource = workflow.replace(
  ...
  await context.test(
    "required aggregate omission remains forbidden after an inventory hash refresh",
    async () => {
      const changedSource = workflow
        .replace(
          "needs: [inventory-status, workspace-verification, plugin-linux, registry-smoke, runtime-portable]",
          "needs: [inventory-status, workspace-verification, registry-smoke, runtime-portable]",
        )
        .replace(
          "          PLUGIN_LINUX: ${{ needs.plugin-linux.result }}\n",
          "",
        )
      ...
      changedInventory.workflows[0].sha256 = createHash("sha256")
        .update(changedSource)
        .digest("hex");
      const gate = changedInventory.workflows[0].jobs.find(
        ({ id }) => id === "required-gate",
      );
      gate.needs = gate.needs.filter(
        (id) => !["plugin-linux", "runtime-network-denied"].includes(id),
      );
      gate.commands = gate.commands.filter(
        (command) =>
          ![
            'test "$PLUGIN_LINUX" = success',
            'test "$RUNTIME_NETWORK_DENIED" = success',
          ].includes(command),
      );
      await rejects(
        () =>
  ...
  await context.test(
    "an explicitly inventoried approved job read scope remains allowed",
    async () => {
      const changedSource = workflow.replace(
        "  inventory-status:\n    name:",
      ...
      changedInventory.workflows[0].sha256 = createHash("sha256")
        .update(changedSource)
        .digest("hex");
      changedInventory.workflows[0].jobs.find(
        ({ id }) => id === "inventory-status",
      ).permissions = { checks: "read" };
      assert.equal(
...
test("dependency, Cargo target, and required record additions and omissions fail closed", async (context) => {
  ...
  await context.test("inventoried mutable dependency protocol", async () => {
    ...
    const changed = structuredClone(inventory);
    changed.packages
      .find(({ path: packagePath }) => packagePath === "packages/ui")
      .dependencies.push({
  ...
  await context.test(
    "new trusted dependency requires explicit review",
    async () => {
      const manifest = JSON.parse(
        await repository.read("packages/ui/package.json"),
      );
  ...
  await context.test(
    "a lifecycle hook cannot be laundered through a scripts inventory refresh",
    async () => {
      const manifest = JSON.parse(
        await repository.read("packages/ui/package.json"),
      );
      ...
      changed.packages.find(
        ({ path: packagePath }) => packagePath === "packages/ui",
  ...
  await context.test(
    "an inventoried but unreviewed trusted dependency remains forbidden",
    async () => {
      const manifest = JSON.parse(
        await repository.read("packages/ui/package.json"),
      );
  ...
  await context.test(
    "unsafe patch protocols remain forbidden after review",
    async () => {
      const manifest = JSON.parse(
        await repository.read("packages/ui/package.json"),
      );
      manifest.patchedDependencies = {
        "react@19.2.0": "https://example.invalid/react.patch",
      };
      const changed = structuredClone(inventory);
      changed.packages
        .find(({ path: packagePath }) => packagePath === "packages/ui")
  ...
  await context.test("implicit Cargo bin", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/runtime/native/src/bin/unreviewed.rs": "fn main() {}\n",
          }),
        ),
      "VERIFICATION_CARGO_SURFACE",
    ),
...
test("Bun execution policy disables implicit trust and every CI install ignores scripts", async (context) => {
  ...
  await context.test(
    "default-trusted msgpackr-extract cannot be laundered through reviewed inventory metadata",
    async () => {
      const changedManifest = structuredClone(manifest);
      changedManifest.trustedDependencies = ["msgpackr-extract"];
      const changed = structuredClone(inventory);
      const rootPackage = changed.packages.find(
        ({ path: packagePath }) => packagePath === ".",
      );
      rootPackage.executionMetadata = [
        ...rootPackage.executionMetadata.filter(
          ({ kind }) =>
            kind !== "trust-policy" && kind !== "trusted-dependency",
        ),
        {
          kind: "trust-policy",
          path: "trustedDependencies",
  ...
  await context.test(
    "workflow flag removal remains forbidden after an inventory hash refresh",
    async () => {
      const changedSource = workflow.replace(
      ...
      const changed = structuredClone(inventory);
      changed.workflows[0].sha256 = createHash("sha256")
        .update(changedSource)
        .digest("hex");
      await rejects(
        () =>
  ...
  await context.test(
    "Bun install aliases and wrappers remain forbidden after a workflow hash refresh",
    async () => {
      const { classifyWorkflowBunInstallCommand } =
        await import("./inventory-model.mjs");
      ...
      assert.equal(
        classifyWorkflowBunInstallCommand("echo install dependencies"),
        "none",
      );
      for (const command of [
        ...
        assert.equal(
          classifyWorkflowBunInstallCommand(command),
          "noncanonical",
          command,
        );
...
test("Bun and npm automatic lifecycle authority is inventoried separately from generic scripts", async () => {
  ...
  for (const name of lifecycle) {
    assert.deepEqual(
      packageExecutionMetadata({ scripts: { [name]: `node ${name}.mjs` } }),
      [
        {
  ...
  assert.deepEqual(
    packageExecutionMetadata({ scripts: { prestart: "node prestart.mjs" } }, [
      "server.js",
    ]),
    [
      {
      ...
      {
        kind: "lifecycle-script",
        path: "scripts.prestart",
        value: "node prestart.mjs",
      },
    ],
  );
  ...
  assert.deepEqual(
    packageExecutionMetadata({
      scripts: { "ordinary-check": "node check.mjs" },
    }),
    [],
  );
});
...
test("npm executes standalone prestart before its implicit server.js start", async () => {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), "curiosity-implicit-start-"),
  );
  try {
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        scripts: {
          prestart:
            "node -e \"require('fs').writeFileSync('prestart.marker','ran')\"",
        },
      })}\n`,
    );
    await writeFile(
      path.join(temporary, "server.js"),
      'require("fs").writeFileSync("start.marker", "ran");\n',
    );
    await execute("npm", ["start", "--silent"], { cwd: temporary });
    assert.equal(
      await readFile(path.join(temporary, "prestart.marker"), "utf8"),
      "ran",
    );
    assert.equal(
      await readFile(path.join(temporary, "start.marker"), "utf8"),
      "ran",
    );
  } finally {
...
test("implicit npm execution surfaces cannot be laundered through inventory refreshes", async (context) => {
  ...
  await context.test("implicit binding.gyp install", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, { "packages/ui/binding.gyp": "{}\n" }),
        ),
      "VERIFICATION_PACKAGE_EXECUTION_METADATA",
    ),
  );
...
test("clean frozen Bun installs ignore lifecycle scripts even for a default-trusted package name", async () => {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), "curiosity-bun-install-policy-"),
  );
  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        scripts: {
          install:
            "node -e \"require('fs').writeFileSync('../../lifecycle-marker','ran')\"",
        },
      })}\n`,
    );
    const trusted = await execute("bun", ["pm", "default-trusted"], {
      cwd: temporary,
    });
    ...
    await assert.rejects(
      () => readFile(path.join(temporary, "lifecycle-marker")),
      { code: "ENOENT" },
    );
    assert.equal(
      await readFile(path.join(temporary, "bun.lock"), "utf8"),
      lock,
    );
  } finally {
    await rm(temporary, { recursive: true, force: true });
...
test("required workspace verification lane covers the harness, starter, and configuration workspaces once", async () => {
  const inventory = await load();
  ...
  assert.equal(
    gate.commands.filter(
      (value) => value === 'test "$WORKSPACE_VERIFICATION" = success',
    ).length,
    1,
  );
});
...
test("required profiles close over their command graph and legacy orphan surfaces make no green claim", async (context) => {
  ...
  for (const [profile, reason] of [
    [
      "legacy-orphan-bootstrap",
      "obsolete imported bootstrap harness is retained but not wired into required verification",
    ],
    [
      "legacy-orphan-comprehensive",
      "obsolete imported comprehensive harness is retained but not wired into required verification",
    ],
    [
      "legacy-orphan-consolidation",
      "historical consolidation harness is retained but not wired into required verification",
    ],
    [
      "legacy-orphan-smoke",
      "obsolete imported smoke harness is retained but not wired into required verification",
    ],
    [
      "legacy-orphan-identity",
      "legacy identity scanner is retained but not wired into required verification",
    ],
  ]) {
    assert.equal(
      inventory.testProfiles[profile].disposition,
      "legacy-orphan",
      profile,
    );
  ...
  await context.test(
    "removing a required script edge cannot be laundered through an inventory refresh",
    async () => {
      ...
      const changed = structuredClone(inventory);
      changed.packages.find(
        ({ path: packagePath }) => packagePath === "apps/plugin/opencode2",
      ).scripts = manifest.scripts;
      await rejects(
        () =>
          verifyInventory(
  ...
  await context.test(
    "manual Darwin real-host work cannot leak into portable or required command graphs",
    async () => {
      const manifestPath = "apps/plugin/opencode2/package.json";
      const manifest = JSON.parse(await repository.read(manifestPath));
...
test("verification tool additions and byte drift fail closed", async (context) => {
  const inventory = await load();
  const repository = createFileRepository(ROOT);
  await context.test("new tool", () =>
    rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "tools/verification/unreviewed.mjs": "export {};\n",
          }),
        ),
  ...
  await context.test("network gate drift", async () => {
    const source = await repository.read(
      "apps/runtime/tools/network-denied-linux.sh",
    );
    await rejects(
      () =>
        verifyInventory(
          inventory,
          overlay(repository, {
            "apps/runtime/tools/network-denied-linux.sh": `${source}\n# drift\n`,
          }),
        ),
  ...
  await context.test(
    "plugin verification wrapper plans and entrypoint are code-owned beyond self-hash",
    async (mutationContext) => {
      const wrapperPath =
        "apps/plugin/opencode2/tools/verification-profile.mjs";
      const source = await repository.read(wrapperPath);
      const tool = inventory.verificationTools.find(
        ({ path: toolPath }) => toolPath === wrapperPath,
      );
      assert.ok(tool);
      assert.deepEqual(tool.profiles, ["plugin-linux", "plugin-darwin"]);
      const mutations = [
        [
          "remove portable verification",
          'linux: Object.freeze({ platform: "linux", commands: Object.freeze(["verify"]) })',
          'linux: Object.freeze({ platform: "linux", commands: Object.freeze([]) })',
        ],
        [
          "reverse Darwin plan",
          'commands: Object.freeze(["verify", "test:real-host"])',
          'commands: Object.freeze(["test:real-host", "verify"])',
        ],
        [
          "append Darwin command",
          'commands: Object.freeze(["verify", "test:real-host"])',
          'commands: Object.freeze(["verify", "test:real-host", "extra"])',
        ],
        ["bypass injected executor", "execute(command)", "void command"],
        [
          "bypass CLI entrypoint",
          "verifyPluginProfile(profile)",
          "pluginVerificationProfilePlan(profile)",
        ],
      ];
      for (const [name, from, to] of mutations)
        await mutationContext.test(name, async () => {
          const changedSource = source.replace(from, to);
          assert.notEqual(changedSource, source, name);
          const changed = structuredClone(inventory);
          changed.verificationTools.find(
            ({ path: toolPath }) => toolPath === wrapperPath,
          ...
          await rejects(
            () =>
              verifyInventory(
                changed,
                overlay(repository, { [wrapperPath]: changedSource }),
              ),
...
test("bounded Turbo source globs cannot be weakened by refreshing the task inventory", async () => {
  ...
  for (const task of ["test", "verify"])
    turbo.tasks[task].inputs = turbo.tasks[task].inputs.filter(
      (input) => input !== "$TURBO_ROOT$/apps/runtime/src/**",
    );
  const changed = structuredClone(inventory);
  ...
  await rejects(
    () =>
      verifyInventory(
        changed,
        overlay(repository, { "turbo.json": `${JSON.stringify(turbo)}\n` }),
      ),
...
test("runtime profiles cannot be conflated with M7 or SDK v2", async () => {
  ...
  assert.equal(
    portable.some((file) => file.includes("m7-")),
    false,
  );
  assert.equal(
    networkDenied.some((file) => file.includes("m7-")),
    false,
  );
  assert.equal(
    Object.keys(inventory.testProfiles).includes("sdk-v2-manual"),
    true,
  );
  const workflow = await readFile(
    path.join(ROOT, ".github/workflows/opencode2.yml"),
    "utf8",
  );
  assert.doesNotMatch(
    workflow,
    /m7:(?:test|build)|legacy-memory-node-api-sdk/u,
  );
