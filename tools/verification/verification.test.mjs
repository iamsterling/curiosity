...
test("query types resolve to the authoritative implementation rather than a handwritten declaration", async () => {
  const manifest = JSON.parse(
    await readFile(path.join(ROOT, "apps/runtime/package.json"), "utf8"),
  );
  assert.equal(manifest.exports["./query"].types, "./src/query.ts");
  assert.equal(
    await readFile(path.join(ROOT, "apps/runtime/src/query.d.ts"), "utf8"),
    'export * from "./query.js";\n',
  );
  const declarationConfig = JSON.parse(
    await readFile(path.join(ROOT, "apps/runtime/tsconfig.types.json"), "utf8"),
  );
...
test("current runtime documentation maps the historical query declaration range to authoritative symbols", async () => {
  const index = await readFile(
    path.join(ROOT, "apps/runtime/docs/README.md"),
    "utf8",
  );
  ...
  const research = await readFile(
    path.join(
      ROOT,
      "apps/runtime/docs/research/reverse-engineering-retrieval-memory-systems-2026-08-18.md",
    ),
    "utf8",
  );
...
test("repository exposes an executable staged plugin and runtime contract", async () => {
  ...
  assert.equal(
    root.scripts["contract:runtime-plugin"],
    "node tools/verification/runtime-plugin-contract.mjs",
  );
...
test("registry smoke builds the plugin before executing installed setup evidence", async () => {
  const manifest = JSON.parse(
    await readFile(
      path.join(ROOT, "apps/plugin/opencode2/package.json"),
      "utf8",
    ),
  );
  assert.match(
...
test("versioned runtime compatibility evidence and ABI profiles are exact", async () => {
  const { verifyRuntimeContractEvidence } =
    await import("./runtime-contract-evidence.mjs");
  ...
  const { resolveNativeLibrary, verifyAbiSymbols } =
    await import("./native-abi.mjs");
  assert.throws(
    () =>
      resolveNativeLibrary({
        platform: "linux",
        directory: "/fixture",
        entries: ["libcuriosity_runtime_native.dylib"],
      }),
...
test("verification inventory is separate from capability lifecycle status", async () => {
  const inventory = JSON.parse(
    await readFile(path.join(ROOT, "docs/verification/inventory.json"), "utf8"),
  );
  assert.equal(inventory.schemaVersion, 1);
  assert.equal("capabilities" in inventory, false);
  const historical = inventory.releaseArtifacts.find(
    ({ id }) => id === "m7-historical-artifact",
  );
  assert.equal(
    historical.sourceCommit,
    "0dfc71de02393da9aad37bc753724886c00e323c",
  );
  assert.equal(
    historical.artifactSha256,
...
test("M7 status is split between immutable history and the unqualified source candidate", async () => {
  const status = JSON.parse(
    await readFile(path.join(ROOT, "docs/status/capabilities.json"), "utf8"),
  );
  assert.equal(status.capabilities.length, 28);
  const historical = status.capabilities.find(
    ({ id }) => id === "runtime-m7-historical",
  );
  const candidate = status.capabilities.find(
    ({ id }) => id === "runtime-m7-current",
  );
  assert.equal(historical.status, "Current");
  assert.equal(
    historical.scope.constraints.includes(
...
test("Linux network denial is fail-closed and enforces the exact Rust pin", async () => {
  const source = await readFile(
    path.join(ROOT, "apps/runtime/tools/network-denied-linux.sh"),
    "utf8",
  );
  ...
  for (const packageName of [
    ...
    for (const taskName of ["test", "verify"]) {
      const matches = graph.tasks.filter(
        (task) => task.package === packageName && task.task === taskName,
      );
      assert.equal(matches.length, 1, `${packageName}#${taskName}`);
      assert.doesNotMatch(matches[0].command, /turbo\s+run\s+(?:test|verify)/u);
      const inputs = Object.keys(matches[0].inputs).map((input) =>
        path.posix.normalize(path.posix.join(matches[0].directory, input)),
      );
      for (const required of requiredInputs)
        assert.equal(
          inputs.filter((input) => input === required).length,
          1,
          `${packageName}#${taskName}:${required}`,
        );
    }
  }
});

test("the required workspace command reaches shared authority, exact vendored workspaces, the harness, starter syntax, and package contracts", async () => {
  const typescript = await import("typescript");
  for (const starter of ["apps/docs", "apps/web"]) {
    const manifest = JSON.parse(
      await readFile(path.join(ROOT, starter, "package.json"), "utf8"),
    );
    for (const command of ["lint", "check-types", "build", "test"])
      assert.equal(
        manifest.scripts.verify.match(
          new RegExp(`bun run ${command}(?:\\s|$)`, "gu"),
        )?.length,
        1,
        `${starter}:${command}`,
      );
    const source = await readFile(
      path.join(ROOT, starter, "app/page.tsx"),
      "utf8",
    );
    ...
    const diagnostics =
      typescript.transpileModule(broken, {
  ...
  for (const configuration of [
    ...
    assert.equal(
      JSON.parse(
        await readFile(path.join(ROOT, configuration, "package.json"), "utf8"),
      ).scripts.verify,
      "bun run test",
    );
  const { stdout } = await execute(
  ...
  const taskIds = JSON.parse(stdout)
    .tasks.map(({ taskId }) => taskId)
    .sort();
  assert.deepEqual(taskIds, [
    "@crafty/editor#verify",
    "@crafty/pen-import#verify",
    "@crafty/scene-model#verify",
    "@crafty/scene-renderer#verify",
    "@crafty/scene-store#verify",
    "@curiosity/authority#verify",
