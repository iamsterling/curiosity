...
const ENUMS = {
  ...
  owner: ["repository", "harness", "plugin", "runtime", "scaffolds"],
  product: ["repository", "harness", "plugin", "runtime", "scaffolds"],
  level: [
    "repository",
    "package",
    "private-profile",
...
export const assertEvidenceFragment = (fragment, detail = String(fragment)) => {
  if (
    fragment.length === 0 ||
    fragment !== fragment.normalize("NFKC") ||
    /\p{Cf}/u.test(fragment) ||
    !EVIDENCE_FRAGMENT.test(fragment) ||
    /\s{2,}/u.test(fragment) ||
    normalizeEvidenceText(fragment).length === 0
  )
...
const validateRefs = (value, name) => {
  for (const [index, item] of array(value, name).entries()) {
    ...
    if (
      separator <= 0 ||
      separator !== ref.ref.lastIndexOf("#") ||
      /#L\d+|:line-\d+/u.test(ref.ref)
    )
...
const validateFacet = (item, key, values, name) => {
  ...
  enumeration(
    required(facet, "state", `${name}.${key}`),
    values,
    `${name}.${key}.state`,
  );
  validateRefs(
    required(facet, "refs", `${name}.${key}`),
    `${name}.${key}.refs`,
  );
  return facet;
};
...
const validateGuards = (value, name) => {
  for (const [index, raw] of array(value, name, 1).entries()) {
    ...
    enumeration(
      required(guard, "mode", name),
      ["exact", "absent"],
      `${name}[${index}].mode`,
    );
    if (guard.mode === "exact")
      string(required(guard, "sha256", name), `${name}[${index}].sha256`);
  }
};
...
const validateCapabilityShape = (item, index) => {
  ...
  if (!WAVE_1_CAPABILITY_TITLES[item.id])
    fail("STATUS_CAPABILITY_INVENTORY", item.id);
  for (const owner of array(item.owners, `${name}.owners`, 1))
    enumeration(owner, ENUMS.owner, `${name}.owners`);
  ...
  const observation = validateFacet(
    item,
    "observation",
    ENUMS.observation,
    name,
  ...
  if (assertionCode !== WAVE_1_ASSERTION_CODES[assertion.state])
    fail("STATUS_ASSERTION_CODE", `${item.id}:${String(assertionCode)}`);
  ...
  const qualification = validateFacet(
    item,
    "qualification",
    ENUMS.qualification,
    name,
  ...
  array(
    required(qualification, "platforms", `${name}.qualification`),
    `${name}.qualification.platforms`,
  );
  ...
  enumeration(
    ...
    ["published", "unpublished", "unknown", "not-applicable"],
  ...
  enumeration(
    required(verdict, "decision", `${name}.verdict`),
    ENUMS.verdict,
  ...
  if (!Object.values(WAVE_1_RATIONALE_CODES).includes(rationaleCode))
    fail("STATUS_VERDICT_RATIONALE", `${item.id}:${rationaleCode}`);
  for (const blockerCode of array(item.blockerCodes, `${name}.blockerCodes`))
    if (!WAVE_1_BLOCKER_TEXT[blockerCode])
      fail("STATUS_BLOCKER_CODE", `${item.id}:${String(blockerCode)}`);
...
const validateCurrentFacetSources = (item) => {
  ...
  for (const facet of facets) {
    ...
    const acceptedKinds =
      item.id === "runtime-m7-historical"
        ? ["decision"]
        : CURRENT_FACET_KINDS[facet];
    const appropriatelyGuarded = refs.every(
      (reference) =>
        acceptedKinds.includes(reference.kind) &&
        exactPaths.has(referencePath(reference)),
    );
    if (refs.length === 0 || !appropriatelyGuarded)
      fail("STATUS_CURRENT_FACET_SOURCE", `${item.id}:${facet}`);
    for (const reference of refs) {
      if (seen.has(reference.ref))
        fail("STATUS_CURRENT_FACET_DISTINCT", `${item.id}:${reference.ref}`);
...
const validateCatalogMetadata = (catalog) => {
  ...
  if (required(policy, "code", "policy") !== WAVE_1_POLICY.code)
    fail("STATUS_POLICY", String(policy.code));
  ...
  for (const [index, raw] of array(
    ...
    for (const key of ["path", "name", "kind", "private"])
      required(workspace, key, `workspaces[${index}]`);
    ...
    enumeration(
      ...
      `workspaces[${index}].kind`,
    );
    if (typeof workspace.private !== "boolean")
      fail("STATUS_REQUIRED", `workspaces[${index}].private`);
    if (workspacePaths.has(workspace.path))
      fail("STATUS_DUPLICATE_ID", workspace.path);
  ...
  const composition = object(
    ...
    "sourceContracts.composition",
  );
  const compositionPath = required(
    composition,
    "path",
    "sourceContracts.composition",
  );
  ...
  array(
    required(composition, "features", "sourceContracts.composition"),
    "sourceContracts.composition.features",
    1,
  );
  const entryExport = required(
    composition,
    "entryExport",
    "sourceContracts.composition",
  );
  ...
  repositoryReferencePath(
    entryExport,
    "sourceContracts.composition.entryExport",
  );
  const exportsContract = object(contracts.exports, "sourceContracts.exports");
  const exportsPath = required(
    exportsContract,
    "path",
    "sourceContracts.exports",
  );
  string(exportsPath, "sourceContracts.exports.path");
  assertRepositoryPath(exportsPath, "sourceContracts.exports.path");
  array(
    required(exportsContract, "keys", "sourceContracts.exports"),
    "sourceContracts.exports.keys",
    1,
  );
  const report = object(
    contracts.capabilityReport,
    "sourceContracts.capabilityReport",
  );
  for (const key of ["path", "export", "pinnedVersion", "capabilities"])
    required(report, key, "sourceContracts.capabilityReport");
  ...
  for (const [name, raw] of Object.entries(report.capabilities)) {
    const capability = object(
      raw,
      `sourceContracts.capabilityReport.capabilities.${name}`,
    );
    if (required(capability, "status", name) !== "disabled")
      fail("STATUS_ENUM", `${name}.status`);
  ...
  for (const [index, item] of contracts.adrIndexes.entries())
    assertRepositoryPath(
      item.path,
      `sourceContracts.adrIndexes[${index}].path`,
    );
  for (const [index, item] of contracts.historicalSnapshots.entries())
    assertRepositoryPath(
      item.path,
      `sourceContracts.historicalSnapshots[${index}].path`,
    );
  ...
  for (const [index, rule] of contracts.forbiddenProductPatterns.entries())
    for (const [rootIndex, root] of rule.roots.entries())
      assertRepositoryPath(
        root,
        `sourceContracts.forbiddenProductPatterns[${index}].roots[${rootIndex}]`,
      );
  for (const [index, guarded] of contracts.guardedSources.entries())
    assertRepositoryPath(
      guarded.path,
      `sourceContracts.guardedSources[${index}].path`,
    );
};
...
const validateSemantics = (item, facets) => {
  ...
  const referencedPaths = new Set(
    [
      ...
      refs.map(({ ref }) => ref.slice(0, ref.indexOf("#"))),
    ),
  );
  if (
    !item.guards.some(
      ({ mode, path: guardPath }) =>
        mode === "exact" && referencedPaths.has(guardPath),
    )
  )
    fail("STATUS_GUARD_UNBOUND", item.id);
  if (
    observation.refs.some(({ kind }) => kind === "external") &&
  ...
  if (availability.production !== "disabled")
    fail("STATUS_WAVE1_PRODUCTION_FORBIDDEN", item.id);
  if (availability.deployment !== "disabled")
    fail("STATUS_WAVE1_DEPLOYMENT_FORBIDDEN", item.id);
  if (
    scope.environments.includes("production") ||
    availability.environments.includes("production")
  )
  ...
  if (activeAvailability && availability.environments.length === 0)
    fail("STATUS_AVAILABILITY_ENV_REQUIRED", item.id);
  if (
    (!activeAvailability || observation.state === "absent") &&
    availability.environments.length > 0
  )
    fail("STATUS_DISABLED_AVAILABILITY_ENVIRONMENT", item.id);
  const scopedEnvironments = new Set(scope.environments);
  if (
    availability.environments.some(
      (environment) => !scopedEnvironments.has(environment),
    )
  )
    fail("STATUS_AVAILABILITY_SCOPE", item.id);
  if (observation.state === "unknown" && activeAvailability)
    fail("STATUS_UNKNOWN_ENABLED", item.id);
  if (observation.state === "contradictory" && activeAvailability)
    fail("STATUS_CONTRADICTORY_ENABLED", item.id);
  if (
    qualification.state === "qualified" &&
    item.id !== "runtime-m7-historical" &&
    !qualification.refs.some(({ kind }) => kind === "test")
  )
    fail("STATUS_RECEIPT_NOT_QUALIFICATION", item.id);
  if (availability.publication === "published")
    fail("STATUS_WAVE1_PUBLICATION_FORBIDDEN", item.id);
  if (item.status === "Current") {
    if (observation.state !== "implemented")
      fail("STATUS_CURRENT_IMPLEMENTATION", item.id);
    if (evidence.state !== "sufficient")
      fail("STATUS_CURRENT_EVIDENCE", item.id);
    if (authority.state !== "authorized")
      fail("STATUS_CURRENT_AUTHORITY", item.id);
    if (
      !["composed", "exported", "registry-ready", "private-release"].includes(
        delivery.state,
      )
    )
      fail("STATUS_CURRENT_DELIVERY", item.id);
    if (!["qualified", "not-required"].includes(qualification.state))
      fail("STATUS_CURRENT_QUALIFICATION", item.id);
    if (
      !["enabled", "observation-only"].includes(availability.state) ||
      verdict.decision !== "GO"
    )
  ...
  if (verdict.rationaleCode !== WAVE_1_RATIONALE_CODES[item.status])
    fail(
      ...
      `${item.id}:${String(verdict.rationaleCode)}`,
    );
  const contradictoryOrMissing =
    ["unknown", "contradictory"].includes(observation.state) ||
    ["missing", "contradictory"].includes(evidence.state) ||
    ["contradictory", "unknown"].includes(qualification.state) ||
    (scope.platforms.length > 0 &&
      (qualification.state !== "qualified" ||
        !same(scope.platforms, qualification.platforms)));
  const authorityFailsClosed = [
    "unauthorized",
    "forbidden",
    "unknown",
  ...
  const retiredForbidden =
    item.status === "Retired" &&
    authority.state === "forbidden" &&
    !contradictoryOrMissing;
  if (
    item.status !== "Current" &&
    !retiredForbidden &&
    (contradictoryOrMissing || authorityFailsClosed) &&
  ...
  if (item.status === "Experimental") {
    if (
      !["implemented", "partial"].includes(observation.state) ||
      !["conditional", "internal", "test-only"].includes(delivery.state)
    )
      fail("STATUS_EXPERIMENTAL_SCOPE", item.id);
    if (availability.state === "enabled" || verdict.decision !== "CONDITIONAL")
      fail("STATUS_EXPERIMENTAL_ENABLEMENT", item.id);
  }
  if (item.status === "Deferred") {
    if (
      availability.state !== "disabled" ||
      availability.production !== "disabled" ||
      availability.deployment !== "disabled" ||
      verdict.decision !== "NO-GO"
    )
      fail("STATUS_DEFERRED_ENABLEMENT", item.id);
    if (item.blockerCodes.length === 0)
      fail("STATUS_DEFERRED_BLOCKER", item.id);
  }
  if (item.status === "Retired") {
    if (
      observation.state !== "absent" ||
      assertion.state !== "negative" ||
      authority.state !== "forbidden" ||
      delivery.state !== "retired" ||
      availability.state !== "absent" ||
      verdict.decision !== "RETIRED"
    )
...
export const validateCatalog = (value) => {
  ...
  if (catalog.$schema !== "./schema.json")
    fail("STATUS_ENUM", `$schema=${String(catalog.$schema)}`);
  if (catalog.schemaVersion !== 1)
    fail("STATUS_ENUM", `schemaVersion=${String(catalog.schemaVersion)}`);
  ...
  if (!same(observedIds, requiredIds))
    fail(
      ...
      `${observedIds.join(",")} != ${requiredIds.join(",")}`,
    );
