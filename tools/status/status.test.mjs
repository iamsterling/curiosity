...
const SDK_ACCEPTANCE_RECEIPT =
  "apps/runtime/docs/specifications/legacy-memory-node-api-sdk-v2-r4-acceptance-receipt.json";
...
const overlay = (base, changes, historicalChanges = {}) => ({
  ...
  exists: async (relative) => {
    ...
    return relative in changes
      ? changes[relative] !== null
      : base.exists(relative);
  },
  read: async (relative) => {
    base.assertPath(relative);
  ...
  listFiles: async (relative) => {
    ...
    for (const [file, value] of Object.entries(changes)) {
      ...
      if (file === relative || file.startsWith(`${relative}/`))
        value === null ? files.delete(file) : files.add(file);
    }
    return [...files].sort();
  },
  historicalIdentity: async (relative) => {
    ...
    const identity = {
      ...(await base.historicalIdentity(relative)),
      ...historicalChange,
    };
    ...
    return {
      ...identity,
      worktreeChanged: normalized(changed) !== normalized(original),
    };
  },
});
...
test("root README is Curiosity-authored and contains canonical status markers", async () => {
  ...
  for (const marker of ["workspace", "status"])
    assert.match(
      readme,
      new RegExp(`<!-- status:${marker}:(?:start|end) -->`, "gu"),
    );
});

test("workspace catalog identifies the independent custom harness", async () => {
  const value = await catalog();
  assert.deepEqual(
    value.workspaces.find(
      ({ path: workspacePath }) => workspacePath === "apps/custom-harness",
    ),
    {
      path: "apps/custom-harness",
  ...
  const capability = value.capabilities.find(
    ({ id }) => id === "custom-harness-vertical-slice",
  );
  assert.deepEqual(capability?.owners, ["harness"]);
  assert.equal(capability?.status, "Current");
  assert.deepEqual(capability?.verdict, {
    decision: "GO",
    rationaleCode: "scope-qualified-current",
  });
});

test("schema validation rejects missing fields and invalid enums", async (context) => {
  await context.test("required owner", () =>
    invalidCatalog(
      (value) => delete value.capabilities[0].owners,
      "STATUS_REQUIRED",
    ),
  );
  await context.test("status enum", () =>
    invalidCatalog(
      (value) => (value.capabilities[0].status = "Ready"),
      "STATUS_ENUM",
    ),
  );
  await context.test("additional capability property", () =>
    invalidCatalog(
      (value) => (value.capabilities[0].publicationClaim = true),
      "STATUS_SCHEMA",
    ),
  );
  await context.test("bad environment array element", () =>
    invalidCatalog(
      (value) => value.capabilities[0].scope.environments.push("prod-ish"),
      "STATUS_SCHEMA",
    ),
  );
  await context.test("bad reference kind enum", () =>
    invalidCatalog(
      (value) => (value.capabilities[0].evidence.refs[0].kind = "proof"),
      "STATUS_ENUM",
    ),
  );
  await context.test(
    "schema independently rejects unsafe reference and guard paths",
    async () => {
      const Ajv2020 = (await import("ajv/dist/2020.js")).default;
      const schema = JSON.parse(
        await readFile(path.join(ROOT, "docs/status/schema.json"), "utf8"),
      );
...
test("repository evidence paths are confined and normalized", async (context) => {
  const { createFileRepository, verifySourceContracts } =
    await implementation();
  for (const unsafe of UNSAFE_REPOSITORY_PATHS)
    await context.test(
      `semantic reference rejection: ${JSON.stringify(unsafe)}`,
      () =>
  ...
  await context.test(
    "verifier rejects an absolute reference before host access",
    async () => {
      const value = await catalog();
  ...
  await context.test(
    "test overlay rejects unsafe virtual paths before access",
    async () => {
      const repository = overlay(createFileRepository(ROOT), {
        "/etc/hosts": "virtual host bytes\n",
      });
      await rejectsWith(
        () => repository.read("/etc/hosts"),
        "STATUS_PATH_INVALID",
      );
    },
  );
  await context.test(
    "filesystem adapter rejects symlink escape and non-regular targets",
    async () => {
      const fixture = await mkdtemp(
        path.join(os.tmpdir(), "curiosity-status-path-"),
      );
      try {
        await mkdir(path.join(fixture, "nested"));
        await mkdir(path.join(fixture, "outside"));
        await writeFile(
          path.join(fixture, "outside", "evidence.txt"),
          "outside evidence\n",
        );
        await symlink("/etc/hosts", path.join(fixture, "nested", "escape"));
        await symlink(
          path.join(fixture, "outside"),
          path.join(fixture, "nested", "escape-parent"),
        );
        const repository = createFileRepository(fixture);
        await rejectsWith(
          () => repository.read("nested/escape"),
          "STATUS_PATH_SYMLINK",
        );
        await rejectsWith(
          () => repository.read("nested/escape-parent/evidence.txt"),
          "STATUS_PATH_SYMLINK",
        );
        await rejectsWith(
          () => repository.read("nested"),
          "STATUS_PATH_NOT_REGULAR",
        );
      } finally {
  ...
  await context.test(
    "filesystem adapter accepts a valid nested regular file",
    async () => {
      const fixture = await mkdtemp(
        path.join(os.tmpdir(), "curiosity-status-path-"),
      );
      try {
        await mkdir(path.join(fixture, "nested", "deeper"), {
          recursive: true,
        });
        await writeFile(
          path.join(fixture, "nested", "deeper", "evidence.txt"),
          "bounded evidence\n",
        );
        const repository = createFileRepository(fixture);
        assert.equal(
          await repository.read("nested/deeper/evidence.txt"),
          "bounded evidence\n",
        );
      } finally {
        await rm(fixture, { recursive: true, force: true });
...
test("recursive discovery fails closed on unexpected symlinks", async (context) => {
  ...
  const fixture = await mkdtemp(
    path.join(os.tmpdir(), "curiosity-status-discovery-"),
  );
  try {
    ...
    await writeFile(
      path.join(fixture, "scan", "nested", "inside.txt"),
      "inside\n",
    );
    const repository = createFileRepository(fixture);
    ...
    await context.test(
      "explicit ignored directory remains untraversed",
      async () => {
        await mkdir(path.join(fixture, "scan", "node_modules"));
        await symlink(
          path.join(fixture, "outside", "outside.txt"),
          path.join(fixture, "scan", "node_modules", "ignored-link"),
        );
        assert.deepEqual(await repository.listFiles("scan"), [
          "scan/nested/inside.txt",
        ]);
      },
    );
  } finally {
...
test("generated writes are root-confined atomic replacements", async (context) => {
  ...
  const fixture = async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "curiosity-status-write-"),
    );
    ...
    await writeFile(
      path.join(root, "docs", "status", "current.md"),
      "old current\n",
      { mode: 0o644 },
    );
    return root;
  ...
  await context.test("exact allowlist only", async () => {
    ...
    try {
      ...
      await rejectsWith(
        () => writeGeneratedOutputs(changed, root),
        "STATUS_GENERATED_WRITE_ALLOWLIST",
      );
      assert.equal(
  ...
  await context.test("symlinked root cannot modify target bytes", async () => {
    ...
    try {
      ...
      await rejectsWith(
        () => writeGeneratedOutputs(outputs(), link),
        "STATUS_GENERATED_WRITE_SYMLINK",
      );
      assert.equal(
        await readFile(path.join(root, "README.md"), "utf8"),
        "old readme\n",
  ...
  await context.test(
    "target symlink cannot modify external bytes",
    async () => {
      const root = await fixture();
      const external = path.join(
        path.dirname(root),
        `${path.basename(root)}-external.md`,
      );
      try {
        ...
        await rejectsWith(
          () => writeGeneratedOutputs(outputs(), root),
          "STATUS_GENERATED_WRITE_SYMLINK",
        );
  ...
  await context.test(
    "parent symlink cannot modify external bytes",
    async () => {
      const root = await fixture();
      const external = await mkdtemp(
        path.join(os.tmpdir(), "curiosity-status-write-external-"),
      );
      try {
        await mkdir(path.join(external, "status"));
        await writeFile(
          path.join(external, "status", "current.md"),
          "external parent target\n",
        );
        ...
        assert.equal(
          await readFile(path.join(external, "status", "current.md"), "utf8"),
          "external parent target\n",
        );
      } finally {
        await rm(root, { recursive: true, force: true });
  ...
  await context.test(
    "successful replacement is bounded and leaves no temp",
    async () => {
      const root = await fixture();
      try {
        ...
        assert.equal(
          (await stat(path.join(root, "README.md"))).mode & 0o777,
          0o644,
        );
        assert.deepEqual(
          (await readdir(root)).filter((name) =>
            name.includes(".status-write-"),
          ),
          [],
        );
        assert.deepEqual(
          (await readdir(path.join(root, "docs", "status"))).filter((name) =>
            name.includes(".status-write-"),
          ),
          [],
        );
  ...
  await context.test("rename fault cleans exclusive temp", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        (await readdir(root)).filter((name) => name.includes(".status-write-")),
        [],
      );
      assert.deepEqual(
        (await readdir(path.join(root, "docs", "status"))).filter((name) =>
  ...
  for (const id of [
    ...
    await context.test(`omitted ${id}`, () =>
      invalidCatalog(
        ...
        "STATUS_CAPABILITY_INVENTORY",
      ),
    );
  await context.test("duplicate", () =>
    invalidCatalog(
      (value) =>
        value.capabilities.push(structuredClone(value.capabilities[0])),
      "STATUS_DUPLICATE_ID",
    ),
  );
  await context.test("unknown replacement", () =>
    invalidCatalog(
      (value) => (value.capabilities[0].id = "unreviewed-capability"),
      "STATUS_CAPABILITY_INVENTORY",
    ),
  );
});
...
test("capability guards must bind to that record's declared evidence", () =>
  invalidCatalog((value) => {
    const unrelated = value.capabilities.find(
      ({ id }) => id === "starter-scaffolds",
    ).guards[0];
...
test("Current requires sufficient evidence and authority", async (context) => {
  const current = (value) =>
    value.capabilities.find(({ id }) => id === "runtime-m1");
  await context.test("missing evidence", () =>
    invalidCatalog(
      (value) => (current(value).evidence.state = "missing"),
      "STATUS_CURRENT_EVIDENCE",
    ),
  );
  await context.test("contradictory evidence", () =>
    invalidCatalog(
      (value) => (current(value).evidence.state = "contradictory"),
      "STATUS_CURRENT_EVIDENCE",
    ),
  );
  await context.test("unauthorized authority", () =>
    invalidCatalog(
      (value) => (current(value).authority.state = "unauthorized"),
      "STATUS_CURRENT_AUTHORITY",
    ),
  );
});
...
test("consequential contradictions and missing authority fail closed", async (context) => {
  ...
  await context.test("platform qualification missing", () =>
    invalidCatalog((value) => {
      const item = value.capabilities.find(
        ({ id }) => id === "plugin-private-runtime-search",
      );
      item.status = "Experimental";
      item.delivery.state = "conditional";
      item.availability.state = "conditional";
      item.availability.environments = [...item.scope.environments];
      item.verdict = {
        decision: "CONDITIONAL",
        rationaleCode: "bounded-experimental",
      };
  ...
  await context.test(
    "implemented but unauthorized Deferred remains representable",
    () =>
      validCatalog((value) => {
        const item = value.capabilities.find(
          ({ id }) => id === "plugin-engineering-intent",
        );
        ...
        item.verdict = {
          decision: "NO-GO",
          rationaleCode: "fail-closed-deferred",
        };
      }),
  );
  await context.test(
    "missing platform qualification remains orthogonal when Deferred",
    () =>
      validCatalog((value) => {
        const item = value.capabilities.find(
          ({ id }) => id === "plugin-private-runtime-search",
...
test("availability is scope-qualified and consequential unknowns fail closed", async (context) => {
  ...
  await context.test("availability environment outside scope", () =>
    invalidCatalog(
      (value) => first(value).availability.environments.push("test"),
      "STATUS_AVAILABILITY_SCOPE",
    ),
  );
  await context.test("production enabled outside production scope", () =>
    invalidCatalog(
      (value) => (first(value).availability.production = "enabled"),
  ...
  await context.test(
    "production availability with disabled production flag",
    () =>
      invalidCatalog((value) => {
  ...
  await context.test("observation-only availability cannot be nowhere", () =>
    invalidCatalog((value) => {
      const item = value.capabilities.find(
        ({ id }) => id === "plugin-hooks-event-capture",
      );
  ...
  await context.test(
    "observation-only availability is active in declared non-production scope",
    () =>
      validCatalog((value) => {
        value.capabilities.find(
          ({ id }) => id === "plugin-hooks-event-capture",
        ).availability.state = "observation-only";
      }),
  );
  await context.test("disabled availability cannot name an environment", () =>
    invalidCatalog((value) => {
      value.capabilities.find(
        ({ id }) => id === "runtime-unified-evidence",
  ...
  await context.test("absent availability is valid only nowhere", () =>
    validCatalog((value) => {
      value.capabilities.find(
        ({ id }) => id === "retired-daemon",
      ).availability.state = "absent";
    }),
  );
  for (const [facet, state] of [
    ["evidence", "partial"],
    ["authority", "limited"],
  ...
  await context.test("deployment enabled outside production scope", () =>
    invalidCatalog(
      (value) => (first(value).availability.deployment = "enabled"),
  ...
  for (const [id, field] of [
    ...
    await context.test(`${id} unknown ${field}`, () =>
      invalidCatalog((value) => {
        value.capabilities.find((item) => item.id === id).availability[field] =
          "unknown";
      }, "STATUS_AVAILABILITY_UNKNOWN"),
    );
  await context.test(
    "unknown authority is representable only after actual fail-closed values and blocker",
    () =>
      validCatalog((value) => {
        const item = value.capabilities.find(
          ({ id }) => id === "plugin-search-surface",
  ...
  await context.test(
    "unknown observation is representable only as disabled Deferred with a blocker",
    () =>
      validCatalog((value) => {
        const item = first(value);
  ...
  await context.test("Deferred requires an explicit blocker", () =>
    invalidCatalog((value) => {
      value.capabilities.find(
        ({ id }) => id === "runtime-sdk-v2",
      ).blockerCodes = [];
    }, "STATUS_DEFERRED_BLOCKER"),
  );
  await context.test(
    "schema closes production environments and flags",
    async () => {
      const schema = JSON.parse(
        await readFile(path.join(ROOT, "docs/status/schema.json"), "utf8"),
      );
      assert.doesNotMatch(
        JSON.stringify(schema.$defs.scope.properties.environments.items),
        /production/u,
      );
      assert.doesNotMatch(
        JSON.stringify(
          schema.$defs.capability.properties.availability.properties
            .environments.items,
        ),
        /production/u,
      );
      assert.deepEqual(
        schema.$defs.capability.properties.availability.properties.production,
        { const: "disabled" },
      );
      assert.deepEqual(
        schema.$defs.capability.properties.availability.properties.deployment,
        { const: "disabled" },
      );
    },
  );
...
test("Current facets require distinct appropriately guarded sources", async (context) => {
  ...
  await context.test("evidence cannot use an observation source", () =>
    invalidCatalog(
      (value) =>
        (first(value).evidence.refs = structuredClone(
          first(value).observation.refs,
        )),
      "STATUS_CURRENT_FACET_SOURCE",
    ),
  );
  await context.test("authority cannot use an observation source", () =>
    invalidCatalog(
      (value) =>
        (first(value).authority.refs = structuredClone(
          first(value).observation.refs,
        )),
      "STATUS_CURRENT_FACET_SOURCE",
    ),
  ...
  await context.test(
    "guard from another capability cannot bind Current evidence",
    () =>
      invalidCatalog((value) => {
        const item = first(value);
        item.evidence.refs = structuredClone(
          value.capabilities.find(
            ({ id }) => id === "plugin-hooks-event-capture",
          ).evidence.refs,
        );
  ...
  await context.test(
    "deleting the exact evidence guard unbinds the facet",
    () =>
      invalidCatalog((value) => {
        const item = first(value);
        const evidencePath = item.evidence.refs[0].ref.split("#")[0];
        item.guards = item.guards.filter(
          ({ path: guardPath }) => guardPath !== evidencePath,
        );
      }, "STATUS_CURRENT_FACET_SOURCE"),
  );
  ...
  for (const facet of ["evidence", "authority", "delivery", "qualification"])
    await context.test(`external ${facet} cannot establish Current`, () =>
      invalidCatalog((value) => {
        first(value)[facet].refs = [
          { kind: "external", ref: `external.example/review#${facet}` },
        ];
      }, "STATUS_CURRENT_FACET_SOURCE"),
    );
  await context.test(
    "external observation must remain unknown and fail closed",
    () =>
      invalidCatalog((value) => {
        first(value).observation.refs = [
          { kind: "external", ref: "external.example/report#observation" },
        ];
...
test("platform claims require exact qualification", () =>
  invalidCatalog((value) => {
    const item = value.capabilities.find(({ id }) => id === "runtime-m1");
    item.scope.platforms = ["darwin-arm64"];
    item.qualification.platforms = [];
...
test("receipts alone cannot promote qualification", () =>
  invalidCatalog((value) => {
    ...
    item.qualification.refs = [
      {
        kind: "receipt",
        ref: "apps/runtime/docs/approvals/legacy-memory-node-api-sdk-v2-r4.json#candidate",
      },
    ];
...
test("Wave 1 mechanically forbids publication promotion", async (context) => {
  await context.test("registry readiness", () =>
    invalidCatalog((value) => {
      const item = value.capabilities.find(
        ({ id }) => id === "plugin-registry-readiness",
      );
  ...
  await context.test("bogus external publication marker", () =>
    invalidCatalog((value) => {
      ...
      item.assertion.refs.push({
        kind: "external",
        ref: "registry.example/package#published",
      });
      item.availability.publication = "published";
    }, "STATUS_WAVE1_PUBLICATION_FORBIDDEN"),
  );
...
test("source contracts detect workspace, composition, export, and capability report mismatches", async (context) => {
  ...
  const verify = (changes, code) =>
    rejectsWith(
      () => verifySourceContracts(value, overlay(repository, changes)),
      code,
    );
  const rootPackage = JSON.parse(await repository.read("package.json"));
  rootPackage.workspaces = rootPackage.workspaces.filter(
    (item) => item !== "apps/plugin/*",
  );
  await context.test("workspace", () =>
    verify(
      { "package.json": `${JSON.stringify(rootPackage)}\n` },
      "STATUS_WORKSPACE_MISMATCH",
    ),
  );
  const plugin = await repository.read(
    "apps/plugin/opencode2/src/plugin/plugin.ts",
  );
  await context.test("composition", () =>
    verify(
      {
        "apps/plugin/opencode2/src/plugin/plugin.ts": plugin.replace(
          "structuredToolsFeature",
          "missingFeature",
        ),
      },
      "STATUS_COMPOSITION_MISMATCH",
    ),
  );
  const pluginPackage = JSON.parse(
    await repository.read("apps/plugin/opencode2/package.json"),
  );
  delete pluginPackage.exports["./server"];
  await context.test("exports", () =>
    verify(
      {
        "apps/plugin/opencode2/package.json": `${JSON.stringify(pluginPackage)}\n`,
      },
      "STATUS_EXPORT_MISMATCH",
    ),
  );
  const report = await repository.read(
    "apps/plugin/opencode2/src/platform/real-host/index.ts",
  );
  await context.test("capability report", () =>
    verify(
      {
        "apps/plugin/opencode2/src/platform/real-host/index.ts": report.replace(
          'reload: { status: "disabled"',
          'reload: { status: "enabled"',
        ),
      },
      "STATUS_CAPABILITY_REPORT_MISMATCH",
    ),
  );
});
...
test("every Wave 1 capability has a fail-closed exact source contract", async (context) => {
  ...
  const value = await catalog();
  const repository = createFileRepository(ROOT);
  assert.equal(value.capabilities.length, 28);
  for (const capability of value.capabilities) {
    ...
    await context.test(capability.id, async () => {
      ...
      const code =
        capability.id === "runtime-m7-historical"
...
test("M5 endpoint and policy mutations invalidate the runtime-m5 review", async (context) => {
  ...
  const policy = await repository.read(
    "apps/runtime/tests/repository-search.test.ts",
  );
  await context.test("endpoint", () =>
    rejectsWith(
      () =>
        verifySourceContracts(
          value,
          overlay(repository, {
            "apps/runtime/src/repository-search.ts": source.replace(
              "https://search.formerhuman.com/agent-search",
              "https://search.example/agent-search",
            ),
          }),
        ),
      "STATUS_SOURCE_CHANGED",
  ...
  await context.test("policy test", () =>
    rejectsWith(
      () =>
        verifySourceContracts(
          value,
          overlay(repository, {
            "apps/runtime/tests/repository-search.test.ts": policy.replace(
              "provider_redirect_rejected",
              "provider_redirect_allowed",
            ),
          }),
        ),
      "STATUS_SOURCE_CHANGED",
...
test("M7 immutable acceptance and changed source candidate cannot be conflated", async () => {
  const { createFileRepository, verifySourceContracts } =
    await implementation();
  const value = await catalog();
  const historical = value.capabilities.find(
    ({ id }) => id === "runtime-m7-historical",
  );
  historical.scope.constraints = historical.scope.constraints.map(
    (constraint) =>
      constraint.startsWith("source commit ")
        ? "source commit de8a4ec674d16d87a184c29b668fc06003caeb6a"
        : constraint,
  );
  await rejectsWith(
    () => verifySourceContracts(value, createFileRepository(ROOT)),
    "STATUS_M7_CONFLATION",
  );
});
...
test("Current qualification evidence must execute through a required verification entrypoint", async () => {
  const { createFileRepository, verifySourceContracts } =
    await implementation();
  const value = await catalog();
  const capability = value.capabilities.find(({ id }) => id === "runtime-m1");
...
test("source contracts detect ADR omissions and preserve the exact historical preflight", async (context) => {
  ...
  const value = await catalog();
  const repository = createFileRepository(ROOT);
  const pluginIndex = await repository.read(
    "apps/plugin/opencode2/docs/architecture/README.md",
  );
      ...
      () =>
        verifySourceContracts(
          ...
          overlay(repository, {
            "apps/plugin/opencode2/docs/architecture/README.md":
              pluginIndex.replace(
                "0031-registry-ready-package-and-black-box-proof.md",
                "omitted.md",
              ),
          }),
        ),
  ...
  const snapshotPath =
    "apps/plugin/opencode2/docs/architecture/preflight-2026-08-12.md";
  ...
  await context.test("historical snapshot deletion", () =>
    rejectsWith(
      () =>
        verifySourceContracts(
          value,
          overlay(repository, { [snapshotPath]: null }),
        ),
    ...
    rejectsWith(
      () =>
        verifySourceContracts(
          value,
          overlay(repository, {
            [snapshotPath]: snapshot.replace(
              "74fe8c51fd2e5fe6b525f95a1b51b1b6f58a4d7d",
  ...
  await context.test("staged rewrite plus catalog rehash", () => {
    ...
    changedCatalog.sourceContracts.historicalSnapshots[0].sha256 = "0".repeat(
      64,
    );
    return rejectsWith(
      () =>
  ...
  await context.test("CRLF-normalized checkout", () =>
    verifySourceContracts(
      value,
      overlay(repository, {
        [snapshotPath]: snapshot.replaceAll("\n", "\r\n"),
      }),
    ),
  );
});
  ...
  await rejectsWith(
    () =>
      verifySourceContracts(
        value,
        overlay(repository, {
          "apps/plugin/opencode2/tools/loopd.mjs": "export {};\n",
        }),
      ),
    "STATUS_RETIRED_SURFACE_PRESENT",
  );
  ...
  await rejectsWith(
    () =>
      verifySourceContracts(
        value,
        overlay(repository, {
          "apps/runtime/src/index.ts": `${source}\n// unclassified change\n`,
        }),
      ),
    "STATUS_SOURCE_CHANGED",
  );
...
test("SDK r4 acceptance receipt is decisive contradictory evidence", async (context) => {
  ...
  const verify = (
    changes,
    code = "STATUS_SDK_RECEIPT_INVALID",
    changedCatalog = value,
  ) =>
    rejectsWith(
      () => verifySourceContracts(changedCatalog, overlay(repository, changes)),
      code,
    );
  await context.test("deleted", () =>
    verify({ [SDK_ACCEPTANCE_RECEIPT]: null }, "STATUS_SDK_RECEIPT_MISSING"),
  );
  ...
  await context.test("decision anchor deleted", () => {
    ...
    item.evidence.refs = item.evidence.refs.filter(
      ({ kind }) => kind !== "decision",
    );
...
test("evidence fragments are meaningful canonical exact-normalized anchors", async (context) => {
  ...
  await context.test("missing exact normalized phrase", async () => {
    ...
    await rejectsWith(
      () => verifySourceContracts(value, repository),
      "STATUS_EVIDENCE_REFERENCE",
    );
  });
  for (const fragment of [
    ...
    await context.test(
      `invalid canonical fragment ${JSON.stringify(fragment)}`,
      () =>
        invalidCatalog((value) => {
          value.capabilities[0].evidence.refs[0].ref = `apps/plugin/opencode2/tests/unit/plugin-entrypoint.test.mjs#${fragment}`;
        }, "STATUS_REFERENCE_FRAGMENT"),
    );
  await context.test(
    "punctuation cannot normalize to an empty match",
    async () => {
      const value = await catalog();
      value.capabilities[0].evidence.refs[0].ref =
        "apps/plugin/opencode2/tests/unit/plugin-entrypoint.test.mjs#---";
...
test("generated current view and canonical README reject drift", async (context) => {
  ...
  assert.deepEqual(
    [...outputs.keys()],
    ["docs/status/current.md", "README.md"],
  );
    ...
    await context.test(target, async () => {
      ...
      await rejectsWith(
        () =>
          checkGeneratedOutputs(
            outputs,
            overlay(repository, { [target]: changed }),
          ),
        "STATUS_GENERATED_DRIFT",
      );
    });
  await context.test(
    "direct README production/publication overclaim",
    async () => {
      ...
      await rejectsWith(
        () => checkGeneratedOutputs(canonicalOutputs, changedRepository),
        "STATUS_GENERATED_DRIFT",
      );
    },
  );
  await context.test("template production/publication overclaim", async () => {
    ...
    await rejectsWith(
      () =>
        renderOutputs(value, overlay(repository, { [templatePath]: changed })),
      "STATUS_TEMPLATE_CLAIM",
    );
  });
  ...
  await context.test("template remains-Current status assertion", async () => {
    ...
    assert.throws(
      () => validateTemplateVisibleText(changed),
      /^STATUS_TEMPLATE_CLAIM(?:$|:)/u,
    );
  });
  ...
  await context.test(
    "template nested list and blockquote overclaims",
    async () => {
      const changed = `${await repository.read("tools/status/README.template.md")}\n> Review\n> - The plugin remains **Current**.\n`;
      assert.throws(
        () => validateTemplateVisibleText(changed),
  ...
  await context.test(
    "extra generated marker pair cannot hide a claim",
    async () => {
      const changed = `${await repository.read("tools/status/README.template.md")}\n<!-- status:status:start -->\nproduction\n<!-- status:status:end -->\n`;
      assert.throws(
        () => validateTemplateVisibleText(changed),
        /^STATUS_TEMPLATE_MARKER(?:$|:)/u,
      );
    },
  );
  ...
  await context.test(
    "unreviewed benign template mutation changes identity",
    async () => {
      const templatePath = "tools/status/README.template.md";
      const changed = `${await repository.read(templatePath)}\nNavigation note.\n`;
      await rejectsWith(
        () =>
          renderOutputs(
            value,
            overlay(repository, { [templatePath]: changed }),
          ),
        "STATUS_TEMPLATE_IDENTITY",
      );
    },
  );
  await context.test(
    "mutable catalog prose is rejected and not rendered",
    async () => {
      const changed = structuredClone(value);
      changed.capabilities[0].assertion.statement =
        "The plugin is published and deployment is enabled.";
  ...
  await context.test("template required neutral structure", async () => {
    const templatePath = "tools/status/README.template.md";
    const changed = (await repository.read(templatePath)).replace(
...
test("new status tooling is Node 18 compatible", async () => {
  const files = [
    "status-generated-write.mjs",
    "status-language.mjs",
    "status-model.mjs",
    "status-paths.mjs",
    "status-registry.mjs",
    "status-render.mjs",
    "status-repository.mjs",
    "status-validation.mjs",
    "status.mjs",
    "status.test.mjs",
  ];
  for (const file of files)
    assert.doesNotMatch(
      await readFile(path.join(ROOT, "tools/status", file), "utf8"),
      /import\.meta\.dirname/u,
      file,
    );
});
