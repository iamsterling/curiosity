...
const metadataPath = resolve(value("--effect-metadata"));
...
await writeJsonExclusive(diagnosticPath, {
  schemaVersion: "custom-harness-q1-r3-source-tree-diagnostic/v1",
...
assert(
  metadata.version === candidates.effect.version,
  "Effect metadata version",
);
assert(
  metadata.dist?.integrity === candidates.effect.lockIntegrity,
  "Effect metadata integrity",
);
const workspaceLock = readFileSync(join(repositoryRoot, "bun.lock"), "utf8");
const workspaceLockVersions = [
  ...workspaceLock.matchAll(/^\s+"effect": \["effect@([^"]+)"/gmu),
].map((match) => match[1]);
...
await writeJsonExclusive(output, {
  schemaVersion: "custom-harness-q1-r3-identity-observation/v3",
  verdict: "PASS",
  ...
  effect: {
    ...
    candidateIntegrity: metadata.dist.integrity,
    workspaceLockVersions,
    workspaceAdoption: workspaceLockVersions.includes(candidates.effect.version)
      ? "PRESENT"
      : "ABSENT",
    workspaceObservation:
      "The isolated Q1 candidate is not adopted by this qualification tranche.",
  ...
  assertions: { passed: 22, failed: 0, skipped: 0 },
});
console.log(
  JSON.stringify({
    verdict: "PASS",
