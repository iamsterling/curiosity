...
const r3Root = resolve(import.meta.dirname, "..");
const scratchRoot =
...
const extendedMetadata = (path) => {
  ...
  return {
    ...
    aclDigest: aclResult.exitCode === 0 ? sha256Bytes(aclResult.stdout) : null,
...
const inventoryEntry = (absolute, displayPath) => {
  ...
  if (stat.isSymbolicLink()) {
    return [
      { ...common, type: "symlink", symlinkTarget: readlinkSync(absolute) },
    ];
  }
  if (stat.isFile()) {
...
if (action === "manifest-q1") {
  ...
  const visit = (absolute) => {
    ...
    if (relativePath === "r3" || relativePath.startsWith(`r3${sep}`)) {
      return;
    }
    const stat = lstatSync(absolute, { bigint: true });
    if (stat.isDirectory()) {
  ...
  visit(q1Root);
  const manifest = {
    schemaVersion: "custom-harness-q1-r3-original-q1-manifest/v1",
    q1Root,
    excluded: ["r3/**"],
  ...
  const result = {
    schemaVersion: "custom-harness-q1-r3-generated-inventory/v1",
    repositoryRoot,
  ...
  const paths = readFileSync(listPath)
    ...
    .filter(
      (path) =>
        ...
        path !== "docs/architecture/custom-harness/qualification/q1/r3" &&
        !path.startsWith(
          "docs/architecture/custom-harness/qualification/q1/r3/",
        ),
    )
    .sort();
  const inventory = inventoryPaths(paths);
  const result = {
    schemaVersion: "custom-harness-q1-r3-ignored-inventory/v1",
    sourceList: listPath,
    excluded: ["node_modules/**", "qualification/q1/r3/**"],
    paths,
    roots: inventory.roots,
    entries: inventory.entries,
    aggregateSha256: sha256Bytes(JSON.stringify(inventory.entries)),
  ...
  for (const path of config.paths) {
    ...
    if (state.state === "absent") {
      if (existsSync(source))
        throw new Error(`baseline absence changed: ${path}`);
  ...
  const result = {
    schemaVersion: "custom-harness-q1-r3-move-aside/v1",
  ...
  const result = {
    schemaVersion: "custom-harness-q1-r3-restore/v1",
    actions,
  ...
  const exact =
    JSON.stringify(beforeNormalized) === JSON.stringify(afterNormalized);
  const result = {
    schemaVersion: "custom-harness-q1-r3-inventory-comparison/v1",
