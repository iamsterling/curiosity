...
export const r2Root = resolve(import.meta.dirname, "..");
...
export const runRecorded = async ({
  ...
  const directory = join(r2Root, "evidence/receipts", safeId);
  ...
  if (forbidden.length > 0) {
    throw new Error(`secret-like environment names rejected: ${forbidden.join(",")}`);
  }
  ...
  const receipt = {
    ...
    knownExclusions,
    verdict:
      processPassed && explicitPassed && filesPassed ? "PASS" : "FAIL",
  };
