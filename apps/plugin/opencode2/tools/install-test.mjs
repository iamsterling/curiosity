...
try {
  ...
  assert.match(helpResult.stdout, new RegExp(`bunx --bun ${expectedPackageSpec.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`))
  assert.doesNotMatch(helpResult.stdout, /latest|npx/u)
  ...
  const configuredSource = JSON.stringify({ plugins: [expectedPackageSpec] })
  await fs.writeFile(path.join(configured, "opencode.json"), configuredSource, "utf8")
  await fs.mkdir(path.join(configured, "plugins", "opencode2-config"), { recursive: true })
  await fs.writeFile(path.join(configured, "plugins", "opencode2-config", "index.js"), "duplicate staged release", "utf8")
  await fs.writeFile(path.join(configured, "plugins", "opencode2-config.receipt.json"), "duplicate receipt", "utf8")
  ...
  assert.equal(await exists(path.join(configured, "plugins", "opencode2-config")), false)
  assert.equal(await exists(path.join(configured, "plugins", "opencode2-config.receipt.json")), false)
  assert.equal(await exists(path.join(configured, "node_modules")), false)
  assert.equal(await exists(path.join(configured, "plugins", "opencode2-config.js")), false)
  ...
  assert.deepEqual(configuredJson.plugins, [expectedPackageSpec])
  assert.equal(await fs.readFile(path.join(configured, "opencode.json"), "utf8"), configuredSource)
