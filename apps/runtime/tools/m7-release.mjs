...
const build = (output) => {
  ...
  try {
    ...
    mkdirSync(join(stage, "runtime")); run("bun", ["build", "apps/runtime/src/query.ts", "--target=bun", "--outfile", join(stage, "runtime/query.js")])
    const typeStage = join(stage, ".runtime-types"); run(join(runtime, "node_modules/.bin/tsc"), ["-p", "apps/runtime/tsconfig.types.json", "--outDir", typeStage]); cpSync(typeStage, join(stage, "runtime/types"), { recursive: true }); rmSync(typeStage)
